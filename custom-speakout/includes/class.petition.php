...
{
    public static function signatures_cache_key( $petition_id, $confirmed_only = false ) {
        $suffix = $confirmed_only ? 'confirmed' : 'all';
        return 'dk_speakout_sigcount_' . absint( $petition_id ) . '_' . $suffix;
    }

    public static function clear_signature_cache( $petition_id ) {
        $petition_id = absint( $petition_id );
        if ( $petition_id < 1 ) {
            return;
        }
        delete_transient( self::signatures_cache_key( $petition_id, false ) );
        delete_transient( self::signatures_cache_key( $petition_id, true ) );
    }
    public $title;
    public $target_email;
...
public function retrieve( $id )
{
    ...
    global $wpdb, $db_petitions, $db_signatures;
    $id = absint( $id );
    $cache_key = self::signatures_cache_key( $id, false );
    $petition = get_transient( $cache_key );
    $rowcount = 0;
        ...
        if ( $petition ) {
            if ( $petition->requires_confirmation != 0 ) {
                $confirmed_sql = "
                    ...
                    GROUP BY $db_petitions.id
                ";
                $petition = $wpdb->get_row( $confirmed_sql );
                set_transient( self::signatures_cache_key( $id, true ), $petition, MINUTE_IN_SECONDS );
            }
            set_transient( $cache_key, $petition, MINUTE_IN_SECONDS );
        }
    } else {
        $rowcount = 1;
    }
