...
class dk_speakout_Signature
{
	...
	public function create( $petition_id, $increase_goal)
	{
		...
		dk_speakout_Petition::clear_signature_cache( $petition_id );
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public function confirm( $confirmation_code )
	{
		...
		if ( $rows_affected > 0 ) {
			$petition_id = $wpdb->get_var(
				$wpdb->prepare( "SELECT petitions_id FROM $db_signatures WHERE confirmation_code = %s", $confirmation_code )
			);
			if ( $petition_id ) {
				include_once( 'class.petition.php' );
				dk_speakout_Petition::clear_signature_cache( $petition_id );
    ...
    */
	...
	{
		global $wpdb, $db_signatures;
		$petition_id = $wpdb->get_var( $wpdb->prepare( "SELECT petitions_id FROM `$db_signatures` WHERE `id` = %d", $signature_id ) );
	...
	public function retrieve_by_code_email( $confirmation_code, $email ) {
		global $wpdb, $db_signatures;
		$sql = "SELECT * FROM `$db_signatures` WHERE `confirmation_code` = %s AND `email` = %s";
		$signature = $wpdb->get_row( $wpdb->prepare( $sql, $confirmation_code, $email ) );
		if ( $signature ) {
			$this->_populate_from_query( $signature );
			return true;
		}
		return false;
	}

	public function update_custom_message( $signature_id, $custom_message ) {
		global $wpdb, $db_signatures;
		$data = array( 'custom_message' => $custom_message );
		$where = array( 'id' => $signature_id );
		$updated = $wpdb->update( $db_signatures, $data, $where );
		if ( $updated !== false ) {
			$petition_id = $wpdb->get_var( $wpdb->prepare( "SELECT petitions_id FROM `$db_signatures` WHERE `id` = %d", $signature_id ) );
			if ( $petition_id ) {
				include_once( 'class.petition.php' );
	...
	public function delete_by_code_email( $confirmation_code, $email ) {
		global $wpdb, $db_signatures;
		$petition_id = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT petitions_id FROM `$db_signatures` WHERE `confirmation_code` = %s AND `email` = %s",
				$confirmation_code,
				$email
			)
		);
		$sql = "DELETE FROM `$db_signatures` WHERE `confirmation_code` = %s AND `email` = %s";
		$deleted = $wpdb->query( $wpdb->prepare( $sql, $confirmation_code, $email ) );
		if ( $deleted ) {
			if ( $petition_id ) {
				include_once( 'class.petition.php' );
				dk_speakout_Petition::clear_signature_cache( $petition_id );
