...
if ( isset( $_REQUEST['csv'] ) && $_REQUEST['csv'] == 'signatures' ) {
	...
	function dk_speakout_signatures_csv() {
        ...
        if ( $date_from || $date_to || $status ) {
            $csv_data = array_filter( $csv_data, function( $row ) use ( $date_from, $date_to, $status ) {
                $row_ts = strtotime( $row->date );
                if ( $date_from && $row_ts < strtotime( $date_from . ' 00:00:00' ) ) {
                    return false;
                }
                if ( $date_to && $row_ts > strtotime( $date_to . ' 23:59:59' ) ) {
                    return false;
                }
                if ( $status === 'confirmed' && (int) $row->is_confirmed !== 1 ) {
                    return false;
                }
                if ( $status === 'unconfirmed' && (int) $row->is_confirmed === 1 ) {
                    return false;
                }
                return true;
            } );
