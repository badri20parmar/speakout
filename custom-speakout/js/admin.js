...
// open or close signature goal settings
    ...
    $('#dk-speakout-bulk-action-form').submit(function () {
        ...
        switch ($('#dk-speakout-bulk-action-form option:selected').val()) {
            case "csv-download":
                var page = 'dk_speakout_signatures',
                    baseurl = String(document.location).split('?'),
                    dateFrom = $('#dk-speakout-csv-date-from').val(),
                    dateTo = $('#dk-speakout-csv-date-to').val(),
                    status = $('#dk-speakout-csv-status').val(),
                    newurl = baseurl[0] + '?page=' + page + '&pid=' + pid + '&csv=signatures&_wpnonce=' + $('#csvnonce').val();
                if (dateFrom) {
                    newurl += '&date_from=' + encodeURIComponent(dateFrom);
                }
                if (dateTo) {
                    newurl += '&date_to=' + encodeURIComponent(dateTo);
                }
                if (status) {
    ...
    // keep save button visible for all tabs in custom build
    $('.button-primary').show();
