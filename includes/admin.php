...
function dk_speakout_create_menus() {
	...
	// load sidebar menus (always enabled in Custom SpeakOut)
	$petitions = array(
		'page_title' => __( 'Custom SpeakOut', 'speakout' ),
		'menu_title' => 'Custom SpeakOut',
	...
	echo '
		<style type="text/css">
			#toplevel_page_dk_speakout .wp-menu-image {
				background: url(' . plugins_url( 'images/icon-emailpetitions-16.png', DK_SPEAKOUT_PLUGIN_FILE ) . ') no-repeat 6px 7px !important;
			}
			body.admin-color-classic #toplevel_page_dk_speakout .wp-menu-image {
				background: url(' . plugins_url( 'images/icon-emailpetitions-16.png', DK_SPEAKOUT_PLUGIN_FILE ) . ') no-repeat 6px -41px !important;
			}
