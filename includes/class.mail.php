...
class dk_speakout_Mail
{
	...
	public static function send_thank_you( $petition, $signature, $options ) {
		...
		$message .= '<p><a href="' . esc_url( $manage_url ) . '">' . esc_html__( 'Edit your comment or remove your signature', 'speakout' ) . '</a></p>';
		$headers  = 'MIME-Version: 1.0' . "\r\n";
		$headers .= 'Content-type: text/html; charset=UTF-8' . "\r\n";
		$headers .= 'From: ' . $options['confirm_email'] . "\r\n";
		self::send( $signature->email, $subject, $message, $headers );
	}
...
/** add email to ActiveCampaign (if enabled) **/
    ...
    public static function add2MailChimp( $email, $firstname, $lastname, $country, $listID, $apiKey, $server){
